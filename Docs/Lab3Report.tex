...
\noindent\textbf{Tagline:} "Learn programming, one paw at a time"

\noindent\textbf{Main Character:} A friendly cartoon cat that navigates through grids, solving programming puzzles. The cat moves with smooth animations and provides visual feedback for successful and failed attempts.

\noindent\textbf{Color Scheme:}
\begin{itemize}
...
\noindent\textbf{Logo:} A stylized cat silhouette with a grid pattern background (see attached logo file).
...
\noindent\textbf{Educational Philosophy:} This dual approach supports scaffolded learning. Beginners can start with visual blocks (no syntax errors possible), gain confidence, then transition to text-based programming when ready. Advanced users can prototype visually then refine in code.

\noindent\textbf{Block Color Coding:}
\begin{itemize}
...
\noindent\textbf{Diagram Organization:} Our UML documentation is organized into specialized views to manage complexity. Figure~\ref{fig:domain} shows core business logic, Figure~\ref{fig:services} shows the service layer, Figure~\ref{fig:ui} shows the original UI components, Figure~\ref{fig:blockeditor} details the block editor subsystem, Figure~\ref{fig:logging} shows the logging infrastructure, and Figure~\ref{fig:completearch} provides a complete architectural overview showing all design patterns.
...
\noindent\textbf{Design Decision:} Grid is optional (nullable) to maintain backward compatibility with Lab 2 programs that run without exercises.
...
\noindent\textbf{Safety Feature:} Includes 10,000 iteration limit to prevent infinite loops.
...
\noindent\textbf{Design Decision:} BlockEditor is a self-contained UserControl that can be easily integrated into any WPF window. It communicates with the main application through events, maintaining loose coupling.
...
\noindent\textbf{Thread Safety:} Uses double-checked locking for singleton initialization, ensuring thread-safe access.

\noindent\textbf{Usage Throughout Application:}
\begin{lstlisting}
// In MainWindow
...
\noindent\textbf{Solution:} Composite pattern with ICommand as component.
...
\noindent\textbf{Enhancement:} RepeatUntilCommand adds conditional execution while maintaining the same interface as RepeatCommand.

\noindent\textbf{Alternative Considered:} Visitor pattern for command execution. Rejected because it would require modifying ICommand interface and complicate the structure for limited benefit.

\subsubsection{Factory Pattern (Updated from Lab 2)}
\textbf{Problem:} Create predefined example programs efficiently.

\noindent\textbf{Solution:} Factory pattern encapsulating object creation.
...
\noindent\textbf{Changes:} Updated to include RepeatUntil commands in advanced and expert examples.

\noindent\textbf{Alternative Considered:} Builder pattern for fluent program construction. Rejected because our programs are simple enough that factory methods are more readable.

\subsubsection{Strategy Pattern (NEW - Bonus Feature)}
\noindent\textbf{Problem:} Support exporting programs to multiple formats (Text, JSON, HTML) with easy extensibility for future formats.

\noindent\textbf{Solution:} Strategy pattern allows selecting export algorithm at runtime.
...
\noindent\textbf{Alternative Considered:} Simple if-else chain in MainWindow. Rejected because it would violate Open/Closed Principle and make adding formats harder.
...
\noindent\textbf{Solution:} Observer pattern allows multiple loggers to observe application events.
...
\noindent\textbf{Alternative Considered:} Direct file writing in MainWindow. Rejected because it tightly couples UI to logging implementation and makes testing difficult.

\noindent\textbf{Extended Implementation Details:}
...
\noindent\textbf{Solution:} Bidirectional Converter pattern with specialized translation logic for each representation.
...
\noindent\textbf{Key Design Principles:}
...
\noindent\textbf{Conversion Mapping:}
...
\noindent\textbf{Testing Strategy:}
...
\noindent\textbf{Alternative Considered:} Direct UI-to-Command conversion without intermediate representation. Rejected because it would tightly couple UI to domain logic, make testing difficult, and violate separation of concerns.
...
\noindent\textbf{New Design:} Created ProgramRunner service to handle execution.

\noindent\textbf{Reason:} Execution became more complex with grid awareness, error handling, and exercise validation. Extracting it improved cohesion and made testing easier.

\subsubsection{Character Grid Awareness}
\textbf{Original Design:} Character had no concept of boundaries or obstacles.

\noindent\textbf{New Design:} Character can have an optional Grid for validation.

\noindent\textbf{Reason:} Required for pathfinding exercises. Made optional to maintain backward compatibility with Lab 2 programs.

\subsubsection{Exception Handling}
\textbf{Original Design:} Generic exceptions for errors.

\noindent\textbf{New Design:} Custom exceptions (OutOfBoundsException, BlockedCellException) with position information.

\noindent\textbf{Reason:} Provides better error messages and makes debugging easier. Allows UI to display specific, helpful feedback to users.
...
\noindent\textbf{How our design handles it:}
\begin{enumerate}
...
\noindent\textbf{Classes affected:}
\begin{itemize}[noitemsep]
...
\noindent\textbf{Difficulty:} Moderate. Our command structure and path tracking provide good foundation. Main challenge is UI synchronization. Estimated: 10-15 hours.
...
\noindent\textbf{Difficulty:} Easy. Composite pattern makes this straightforward. Estimated: 8-10 hours including tests.

\subsubsection{Difficult Changes}

\paragraph{Web-Based Collaborative Editing}

\noindent\textbf{Scenario:} Multiple users editing same program simultaneously in browser.
...
\noindent\textbf{Why difficult:} Fundamental architecture change. WPF is desktop-only. Would need operational transformation or CRDT algorithms for collaboration. State management becomes distributed. Estimated: 100+ hours.

\noindent\textbf{Mitigation:} Our service layer is UI-agnostic, so domain logic could be reused. But UI and synchronization would be complete rewrites.
...
\noindent\textbf{Why difficult:} Current design assumes static grid. Would need reactive programming model. Character and Grid would need much tighter coupling. Condition evaluation becomes stateful. Estimated: 40-50 hours.
...
\noindent\textbf{Why difficult:} Completely different input paradigm. Would need NLP library or ML model. Our structured command system wasn't designed for fuzzy input. Ambiguity resolution requires AI. Estimated: 200+ hours plus ML expertise.
...
\noindent\textbf{Overall:} The design successfully balances simplicity with extensibility. It handles likely future changes well while acknowledging fundamental limitations for more radical changes.
...
\noindent\textbf{Block Editor Testing:} The BlockEditorConverterTests class contains 102 comprehensive tests covering:
\begin{itemize}[noitemsep]
...
\noindent\textbf{Logging System Testing:} The LoggingTests class contains 14 tests covering:
\begin{itemize}[noitemsep]
...
\noindent\textbf{Note:} UI coverage is lower because WPF is tested primarily through manual system tests.
...
\noindent\textbf{Scenario 2: Exercise Completion}
\begin{enumerate}[noitemsep]
...
\noindent\textbf{Scenario 3: Runtime Error Handling}
\begin{enumerate}[noitemsep]
...
\noindent\textbf{Scenario 4: RepeatUntil Execution}
\begin{enumerate}[noitemsep]
...
\noindent\textbf{Scenario 5: Program Export}
\begin{enumerate}[noitemsep]
...
\noindent\textbf{What Didn't Work:}
\begin{itemize}
...
\end{longtable}
\noindent\textbf{Summary:} Tijmen focused more on exercise model, UI foundation, and export features. Ilyas concentrated on grid system, visualization, and logging. Both contributed equally to design, testing, and documentation.
...
We allocated 10 hours for UI, but it took 15 hours. WPF has a steeper learning curve than expected, especially canvas manipulation and coordinate transformations.
...
\noindent\textbf{Ilyas:} Developed stronger testing discipline and learned Observer pattern implementation. Improved code review skills and learned to give constructive feedback.

\noindent\textbf{Both:} Enhanced collaboration skills, learned realistic project planning, and gained appreciation for software engineering principles beyond just coding.
